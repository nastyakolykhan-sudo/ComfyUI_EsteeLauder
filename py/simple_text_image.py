...
from .utils import AnyType, log, get_font_dict, tensor2pil, pil2tensor, image2mask
...
class SimpleTextImage:
    ...
    def INPUT_TYPES(cls):
        ...
        font_list = list(font_dict.keys()) if font_dict else ['no fonts found']
        return {
            "required": {
    ...
    def simple_text_image(self, text, font_file, align, char_per_line,
        ...
        if size_as is not None:
            if size_as.dim() == 2:
                size_as_image = torch.unsqueeze(size_as, 0)
            elif size_as.shape[0] > 0:
        ...
        if not font_path:
            log(f"{self.NODE_NAME}: Font '{font_file}' not found.", message_type='error')
            raise ValueError(f"Font '{font_file}' not found in brand fonts.")
        ...
        for paragraph in paragraphs:
            ...
            for line in lines:
                ...
                if align == "left":
                    ...
                    x_text = img_width - line_width - x_offset
                else:
                    x_text = x_offset
                draw.text(
                    xy=(x_text, y_text),
